...
def hf_audio_passthrough(data, max_duration=45.0, min_duration=1.0, mode="train"):
    """Pass-through filter for HuggingFace datasets where audio is already
    decoded into sample["speech"] and sample["sample_rate"] by the dataset class."""
    for sample in data:
        start = time.time()
        if "speech" not in sample:
            continue
        duration = sample.get(
            "duration", sample["speech"].shape[-1] / sample["sample_rate"]
        )
        if duration < min_duration or duration > max_duration:
            continue
        sample["duration"] = duration
        sample["load_audio_time"] = time.time() - start
        yield sample
